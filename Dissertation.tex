...
\textbf{Boolean logic}\\
Boolean logic is referred to as two valued logic as two values are used to determine if something is true of false. True is represented as 1 and false is 0. The structure of Boolean logic is
\begin{center}
...
The first line notes what the formula function \textit{f} is. The second line shows that $\varphi$ maps a variable through total assignment. \textit{Var}(\textit{f}) is the set of objects to be assigned a value. Lines 4-6 show the $\varphi$ mapping of the variables \textit{a} $\rightarrow$ 1, \textit{b} $\rightarrow$ 1, \textit{c} $\rightarrow$ 0. These values are arbitrary and are chosen at random. \\
